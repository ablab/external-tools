...
#   -Error on Bowtie-build solved by chmod Bowtie files
  ...
  use vars qw($opt_m $opt_o $opt_v $opt_p $opt_k $opt_a $opt_z $opt_s $opt_b $opt_n $opt_l $opt_x $opt_u $opt_t);
  &Getopts('m:o:v:p:k:a:z:s:b:n:l:x:u:t:');
  my ($base_overlap,$min_overlap,$verbose,$MIN_READ_LENGTH,$SEQ_SLIDE,$min_base_ratio,$min_links,$max_link_ratio,$unpaired_file,$max_trim,$base_name, $max_count_trim,$min_tig_overlap, $doplot, $extending)= (20, 32, 0, 16, 1, 1, 5, 0.70, "no-u", 0, "standard_output", 10, 15, 0, 0);
...
#-------------------------------------------------Print input parameters
  my $contig = "intermediate_results/" . $base_name .  ".extendedcontigs.fasta";
  $contig = "intermediate_results/" . $base_name .  ".formattedcontigs.fasta" if($extending == 0);
...
#-------------------------------------------------READING AND CONVERTING INPUT SEQUENCES
  ...
  system("perl $Bin/bin/readLibFiles.pl $libraryfile $base_name $extending $unpaired");

#-------------------------------------------------FORMATTING OR EXTENDING CONTIGS
  
  system("perl $Bin/bin/ExtendOrFormatContigs.pl $contig $base_name $extending $filecontig $MIN_READ_LENGTH $base_overlap $min_overlap $min_base_ratio $max_trim $verbose $Bin");
...
#-------------------------------------------------MAPPING READ PAIRS USING FILTERED FASTA FILE
    my $outfile = "reads/$base_name.$lib.filtered.readpairs.singles.fasta";

    my $up_iz = ($insert_size - $min_allowed);
    mkpath("tmp.$base_name");
    my $newcontig = processContig($contig, $up_iz);
    system("perl $Bin/bin/mapWithBowtie.pl $base_name $newcontig $outfile $lib $Bin");
#-------------------------------------------------Scaffold the contigs and generate .scaffold file
    my $bowtiefile = "bowtieoutput/" . $base_name . ".$lib.mapped";

    if(-e $bowtiefile){
      system("perl $Bin/bin/PairingAndScaffolding.pl $contig $base_name $issues $distribution $verbose $lib $insert_size $min_allowed $scaffold $min_links $max_link_ratio");
