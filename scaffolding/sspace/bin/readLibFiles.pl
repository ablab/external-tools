  ...
  open OUTFILEExt, "> reads/$filenameOutExt" if($extending == 1);
...
#-------------------------------------------------LOOP THROUGH EACH LIBRARY IN LIBRARYFILE AND STORE AND FILTER READS
  ...
  while(<FILELIB>){
    ...
    if($library ne $prevlibrary){
     ...
     open (OUTSINGLEFILE, ">reads/$base_name.$library.filtered.readpairs.singles.fasta") || die "Can't write to single file -- fatal\n";
