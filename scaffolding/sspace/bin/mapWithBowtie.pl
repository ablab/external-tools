...
sub MapReadsToContigs{
    ...
    system("$bowtiepath -v 0 bowtieoutput/$bowtieout --suppress 2,3,4,6,7 -f $singlereads bowtieoutput/$outfileNotExt --un bowtieoutput/$outfileExt --quiet --refidx") == 0 || die "\nBowtie error; $?" if($library eq "start"); # returns exit status values
    system("$bowtiepath -v 0 -m 1 bowtieoutput/$bowtieout --suppress 6,7 -f $singlereads bowtieoutput/$outfileNotExt --quiet --refidx") == 0 || die "\nBowtie error; $?" if($library ne "start"); # returns exit status values
    &FlushFiles();
}
